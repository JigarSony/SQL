...
> MYSQL: define Primary key for MYSQL
...
> SQL Server and ORACLE: define Primary key for SQL Server and ORACLE
...
> );
    ...
    insert into Student values(1, "Jack", "Ma", 25);

> If you're passing duplicate ID - Primary key

    insert into Student values(1, "Tom", "Ma", 35);
    
> UNIQUE constraint failed: Student.ID

> If you remove primary key constraint then it'll allow the duplicate values

    select * from Student;

> How to define for PKC for multiple columns
...
> If creating New table above syntax

    insert into Employee values(1, "Jack", "Ma", 25);
    
    insert into Employee values(2, "Jack", "Ma", 25);

> If you pass same values to ID and LastName

> UNIQUE constraint failed: Employee.ID, Employee.LastName

    select * from Employee;

> If table is already created then below syntax

> Add primary key on Existing table

    Alter table Employee add Primary key (ID);

> Drop/delete Primary key on an Existing table
